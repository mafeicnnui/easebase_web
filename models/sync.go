...
type TDbSyncConfig struct {
	...
	SourDbId       int       `orm:"description(源数据源ID)"`
	DestDbId       int       `orm:"description(目标数据源ID)"`
	ServerId       int       `orm:"description(同步服务器ID)"`
...
type TDbSyncTasksLogDetail struct {
	Id         int
	SyncTag    string    `orm:"size(100);description(同步标识)"`
